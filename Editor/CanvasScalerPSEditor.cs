...
{
    [CustomEditor(typeof(CanvasScalerPS))]
    public class CanvasScalerPSEditor : Editor
    {
        ...
        protected virtual void OnEnable()
        {
            ...
            m_Scales = serializedObject.FindProperty("m_scales");
        ...
        public override void OnInspectorGUI()
        {
            ...
            bool showWorld = ((target as CanvasScalerPS).GetComponent<Canvas>().renderMode == RenderMode.WorldSpace);
            for (int i = 0; i < targets.Length; i++)
            {
                CanvasScalerPS scaler = targets[i] as CanvasScalerPS;
            ...
            if (!showWorldDiffers && !(!showWorld && m_UiScaleMode.hasMultipleDifferentValues))
            {
                ...
                // Constant pixel size
                else if (m_UiScaleMode.enumValueIndex == (int)CanvasScalerPS.ScaleMode.ConstantPixelSize)
                {
                    EditorGUILayout.PropertyField(m_ScaleFactor);
                }
                // Scale with screen size
                else if (m_UiScaleMode.enumValueIndex == (int)CanvasScalerPS.ScaleMode.ScaleWithScreenSize)
                {
                    EditorGUILayout.PropertyField(m_ReferenceResolution);
                    EditorGUILayout.PropertyField(m_ScreenMatchMode);
                    if (m_ScreenMatchMode.enumValueIndex == (int)CanvasScalerPS.ScreenMatchMode.MatchWidthOrHeight && !m_ScreenMatchMode.hasMultipleDifferentValues)
                    {
                ...
                else if (m_UiScaleMode.enumValueIndex == (int)CanvasScalerPS.ScaleMode.ConstantPhysicalSize)
                {
